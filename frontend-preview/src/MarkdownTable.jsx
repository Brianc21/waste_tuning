...
const MarkdownTable = forwardRef(function MarkdownTable({ data, loading, error, maxConfigVersion, activeConfigVersion, onPreviewSql }, ref) {
  ...
  const [showOnlyLeaveRows, setShowOnlyLeaveRows] = useState(false)
  ...
  const [showOnlyUndecidedRows, setShowOnlyUndecidedRows] = useState(false)
...
WHERE VersionID = ${maxVersionID}`
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter(row => {
      ...
      if (showOnlyLeaveRows && rowDecisions[row.PPGClusterID] !== 'Leave') return false
      ...
      if (showOnlyUndecidedRows && rowDecisions[row.PPGClusterID]) return false
      return true
    })
  }, [data, filterHierarchy4, filterHierarchy3, filterHierarchy2, filterHierarchy1, filterPPGCluster, hideLeaveRows, showOnlyLeaveRows, hideChangeRows, showOnlyChangeRows, hideResetRows, showOnlyResetRows, showOnlyUndecidedRows, rowDecisions])
  ...
  return (
    <div className="markdown-table-container">
      ...
      {/* Filter Controls */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '16px', padding: '16px', background: '#f8f9fa', borderRadius: '8px' }}>
        {/* Row 1: dropdowns + search + clear */}
        ...
        {/* Row 2: visibility checkboxes — always below the filters */}
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
          {[
            { id: 'hideLeaveRows', state: hideLeaveRows, setter: setHideLeaveRows, label: 'Hide "Leave" rows', bg: '#d4edda' },
            { id: 'showOnlyLeaveRows', state: showOnlyLeaveRows, setter: setShowOnlyLeaveRows, label: 'Show Only "Leave" rows', bg: '#d4edda' },
            ...
            { id: 'showOnlyUndecidedRows', state: showOnlyUndecidedRows, setter: setShowOnlyUndecidedRows, label: 'Show Only Unworked rows', bg: '#fff3cd' },
