@echo off
echo ======================================================================
echo   HEB Waste Dashboard - Build Script
echo ======================================================================
echo.
...
mkdir "dist\Waste-Tuning-Dashboard"
...
powershell -NoProfile -Command "Expand-Archive -Path '%PYTHON_ZIP%' -DestinationPath 'dist\Waste-Tuning-Dashboard\python' -Force"
...
powershell -NoProfile -Command "$pth = Get-ChildItem 'dist\Waste-Tuning-Dashboard\python\python3*._pth' | Select-Object -First 1; $content = Get-Content $pth.FullName; $content = $content -replace '#import site', 'import site'; Set-Content $pth.FullName $content; Add-Content $pth.FullName 'Lib\site-packages'; Add-Content $pth.FullName '..'"
if errorlevel 1 (
...
mkdir "dist\Waste-Tuning-Dashboard\python\Lib\site-packages"
"%PYTHON_EXE%" -m pip install -r backend\requirements.txt --target "dist\Waste-Tuning-Dashboard\python\Lib\site-packages" --no-user --quiet
if errorlevel 1 (
...
copy "backend\sql_proxy.py" "dist\Waste-Tuning-Dashboard\"
copy "backend\main.py"      "dist\Waste-Tuning-Dashboard\"
copy "backend\db_proxy.py"  "dist\Waste-Tuning-Dashboard\"

REM Create run_proxy.bat helper
REM The launcher passes a single path to runas (matching the original EXE approach).
REM This avoids CMD quoting issues that arise when two full paths are embedded in
REM a runas command string.
(
    echo @echo off
    echo cd /d "%%~dp0"
    echo python\python.exe sql_proxy.py
) > "dist\Waste-Tuning-Dashboard\run_proxy.bat"

REM Create run_dashboard.bat helper
REM Uses cmd /k so the window stays open if main.py crashes, making errors visible.
(
    echo @echo off
    echo cd /d "%%~dp0"
    echo python\python.exe main.py
) > "dist\Waste-Tuning-Dashboard\run_dashboard.bat"

REM Copy static files (React frontend, served by the dashboard)
xcopy "backend\static" "dist\Waste-Tuning-Dashboard\static\" /E /I /Q

REM Copy queries
copy "backend\queries.json" "dist\Waste-Tuning-Dashboard\"

REM Copy launcher
copy "build\launcher_template.bat" "dist\Waste-Tuning-Dashboard\Start Dashboard.bat"

REM Create README for users
(
echo Waste Tuning Dashboard
echo ======================
echo.
...
) > "dist\Waste-Tuning-Dashboard\README.txt"
...
echo ======================================================================
echo.
echo Distribution package created in: dist\Waste-Tuning-Dashboard\
echo.
echo Contents:
dir /b "dist\Waste-Tuning-Dashboard\"
echo.
echo NOTE: config.ini is NOT included - it is auto-created with default settings on first run.
echo.
echo You can now zip and distribute the Waste-Tuning-Dashboard folder.
echo.
pause
